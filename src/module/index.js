...
const content = styles => (
  <View style={styles.container}>
    ...
    <Text style={styles.instructions}>{strings.GET_STARTED}</Text>
    <Text style={styles.instructions}>{strings.POWERED}</Text>
  </View>
);
...
const changeLAng = (context, styles) => (
  <View style={styles.langContainer}>
    ...
    <View style={styles.select}>
      <TouchableOpacity
        style={{ flex: 1, backgroundColor: "#0F05" }}
        onPress={context.props.changeAR}
      >
        <Text style={styles.text}>{strings.AR}</Text>
      </TouchableOpacity>
      <TouchableOpacity
        style={{ flex: 1, backgroundColor: "#F005" }}
        onPress={context.props.changeEN}
      >
